...
node("aws-ecs") {
    timestamps {
        ...
        String ZIP_DIR = currentDir + "/build/distributions"
        ...
        stage("build") {
            ...
            sh "./gradlew :fbiisa-backend:distZip"
        }
        ...
        stage("publish") {
            ...
            withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: 'traveloka-builds-credential-s3', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) {
                sh './gradlew :fbiisa-backend:uploadAmiBakingManifest -Pversion=$(git rev-parse --short HEAD)'
            }
        }
    }
}